...
const activePrayersQuery = defineQuery([Player, ActivePrayers, Skills]);

// Prayer drain rates (points per minute)
const PRAYER_DRAIN_RATES: { [key in PrayerFlag]?: number } = {
  // Protection prayers
...
const PRAYER_REQUIREMENTS: { [key in PrayerFlag]?: number } = {
  [PrayerFlag.THICK_SKIN]: 1,
...
const PRAYER_CONFLICTS: { [key in PrayerFlag]?: PrayerFlag[] } = {
...
export const PrayerSystem = defineSystem((world: IWorld) => {
  ...
  for (let i = 0; i < players.length; i++) {
    ...
    for (const [flag, drainRate] of Object.entries(PRAYER_DRAIN_RATES)) {
      const prayerFlag = Number(flag) as PrayerFlag;
 ...
 */
export function activatePlayerPrayer(world: IWorld, playerId: number, prayer: PrayerFlag): boolean {
 ...
 */
export function deactivatePlayerPrayer(world: IWorld, playerId: number, prayer: PrayerFlag): void {
  const currentPrayers = ActivePrayers.prayers[playerId] || 0;
  ActivePrayers.prayers[playerId] = deactivatePrayer(currentPrayers, prayer);
}
