...
export class RuneRogueRoom extends Room<GameState> {
  ...
  onCreate(options: any) {
    ...
    console.log(`[${new Date().toISOString()}] RuneRogueRoom created with options:`, options);
  ...
  onJoin(client: Client, options: any) {
    console.log(`[${new Date().toISOString()}] Player joined room '${this.roomId}' - Player ID: ${client.sessionId}`);
    console.log(`[${new Date().toISOString()}] Room '${this.roomId}' state - Players: ${this.clients.length}/${this.maxClients}`);
  ...
  onLeave(client: Client, consented: boolean) {
    console.log(`[${new Date().toISOString()}] Player left room '${this.roomId}' - Player ID: ${client.sessionId} (consented: ${consented})`);
    console.log(`[${new Date().toISOString()}] Room '${this.roomId}' state - Players: ${this.clients.length}/${this.maxClients}`);
  ...
  onMessage(client: Client, message: any) {
    console.log(`[${new Date().toISOString()}] Message from ${client.sessionId}:`, message);
