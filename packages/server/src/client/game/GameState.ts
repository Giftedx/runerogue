...
export class GameState {
  ...
  private initializeEmptyRoom(): void {
    ...
    this.tileMap = Array(this.roomHeight)
      .fill(0)
      .map(() => Array(this.roomWidth).fill(0));

    // Create empty collision map
    this.collisionMap = Array(this.roomHeight)
      .fill(false)
      .map(() => Array(this.roomWidth).fill(false));
  ...
  public updateFromServer(serverState: any): void {
    ...
    if (
      serverState.currentMapId &&
      serverState.maps &&
      serverState.maps[serverState.currentMapId]
    ) {
