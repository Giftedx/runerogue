...
async function runLinting(params) {
  ...
  return {
    ...
    output: lintResults.success
      ? `No linting issues found in ${file_path}`
      : `Linting issues found in ${file_path}`,
...
async function generateDocs(params) {
  ...
  return new Promise((resolve, reject) => {
    const process = spawn(
      'python',
      ['scripts/generate_osrs_docs.py', `--module=${module_path}`, `--format=${format}`],
      {
        cwd: PROJECT_ROOT,
        shell: true,
        stdio: 'pipe',
