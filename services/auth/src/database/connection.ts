...
console.log('[ConnectionTS - Restoring Real Connection] dotenv.config() called.');
...
console.log(`[ConnectionTS - Restoring Real Connection] DB_USER from env: ${process.env.DB_USER ? 'loaded' : 'NOT loaded'}`);
...
try {
  ...
  console.log('[ConnectionTS - Restoring Real Connection] New Pool instance created.');

  poolInstance.on('connect', (client) => {
    console.log('[ConnectionTS - Restoring Real Connection] Pool: Client connected.');
    // You could log client.processID here for more details if needed
  });

  poolInstance.on('error', (err, client) => {
    console.error('[ConnectionTS - Restoring Real Connection] Pool: Idle client error:', err.message, err.stack);
    // It's important to handle errors here, e.g., by removing the client from the pool or exiting.
  });

} catch (error) {
  console.error('[ConnectionTS - Restoring Real Connection] FATAL: Error creating Pool instance:', error);
...
export const query = (text: string, params?: any[]) => {
  console.log(`[ConnectionTS - Restoring Real Connection] query function called with text: ${text.substring(0, 50)}...`);
  if (!poolInstance) {
    console.error("[ConnectionTS - Restoring Real Connection] Pool is not initialized!");
    throw new Error("Database pool not initialized. This should not happen if initial setup was successful.");
  }
