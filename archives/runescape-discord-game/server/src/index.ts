...
apiRouter.post(
  ...
  async (req: Request, res: Response) => {
    ...
    const DISCORD_CLIENT_ID =
      process.env.DISCORD_CLIENT_ID || "YOUR_DISCORD_CLIENT_ID_PLACEHOLDER";
    const DISCORD_CLIENT_SECRET =
      process.env.DISCORD_CLIENT_SECRET ||
      "YOUR_DISCORD_CLIENT_SECRET_PLACEHOLDER";
    ...
    const REDIRECT_URI =
      process.env.DISCORD_REDIRECT_URI || "http://127.0.0.1/callback"; // Example placeholder
    ...
    if (
      ...
      return res.status(500).json({
        error: "Server configuration error: Discord credentials missing.",
      });
    }
    ...
    try {
      ...
      console.error(
        "Token Exchange: Error exchanging Discord authorization code for token:",
      );
