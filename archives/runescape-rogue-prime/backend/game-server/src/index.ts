...
const worldGenerator = new WorldGenerator({
  ...
  height: 100,
  biomes: [
    BiomeType.LumbridgePlains,
    BiomeType.VarrockCity,
...
wss.on("connection", async (ws) => {
    ...
    try {
      ...
      switch (parsedMessage.type) {
        ...
        case "player_action":
          ...
          console.log(
            `Player ${player.username} performed action: ${actionMsg.action}`,
          );
        ...
        default:
          console.warn(
            `Unknown message type from ${player.username}: ${parsedMessage.type}`,
          );
      }
    } catch (error) {
  ...
  ws.on("close", async () => {
    ...
    await db.savePlayer(player);
    console.log(
      `Client disconnected: ${player.username}. Total players: ${connectedPlayers.size}`,
    );
  });
