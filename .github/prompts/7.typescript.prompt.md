...
const COMBAT_STYLES = ['accurate', 'aggressive', 'defensive', 'controlled'] as const;
...
export class GameRoomState extends Schema {
  ...
  addPlayer(sessionId: string, playerData: PlayerCreationData): Result<PlayerSchema> {
    ...
    try {
      ...
      return { 
        success: false, 
        error: error instanceof Error ? error : new Error('Failed to create player') 
      };
    }
  }
}
...
export function calculateMaxHit(
  ...
  const effectiveLevel = Math.floor(level * (prayers.strengthMultiplier ?? 1.0));
